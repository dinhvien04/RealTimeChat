...
function initializeChat() {
    ...
    socket.on('message:new', (data) => {
        ...
        // Nếu là tin nhắn audio của mình, xóa pending message trước
...
function appendMessage(data) {
    ...
    if (data.type === 'audio' && data.content) {
        ...
        let imgUrl = data.content;
        if (typeof imgUrl === 'string' && imgUrl.startsWith('/uploads/')) {
            imgUrl = window.location.origin + imgUrl;
        }
        content = `<img src="${imgUrl}" class="chat-image" alt="image" style="max-width:320px;max-height:220px;border-radius:12px;display:block;margin:0 auto;object-fit:cover;" />`;
    } else if (data.type === 'file' && data.content) {
        let fileUrl = data.content;
        if (typeof fileUrl === 'string' && fileUrl.startsWith('/uploads/')) {
            fileUrl = window.location.origin + fileUrl;
        }
        const fileName = data.fileName || fileUrl.split('/').pop() || 'Tệp đính kèm';
        content = `<div style="background:#f0f0f0;padding:10px;border-radius:8px;display:inline-block;">
            <i class="fas fa-file" style="color:#666;margin-right:8px;"></i>
            <span style="color:#333;font-weight:500;">${fileName}</span>
            <a href="${fileUrl}" target="_blank" style="margin-left:10px;color:#007bff;text-decoration:none;">
...
window.sendImagePreview = async function () {
    console.log('Gửi ảnh, previewImageFile:', previewImageFile);
    if (!previewImageFile) {
        showNotification('Không có ảnh để gửi', 'error');
        return;
    }
    ...
    try {
        ...
        if (data.url || data.fileUrl || data.filename) {
            const fileUrl = data.url || data.fileUrl || `/uploads/${data.filename}`;
            const messageData = {
                content: fileUrl,
                type: 'image'
            };
            console.log('Socket emit message:send', messageData);
            socket.emit('message:send', messageData);
        } else {
            console.error('Upload response không có url:', data);
            showNotification('Tải ảnh thất bại', 'error');
        }
    } catch (err) {
        console.error('Lỗi khi fetch upload:', err);
